...
const Revolutionary = () => {
    const [revolutionary, setRevolutionary] = useState([]);
       useEffect(() => {
            fetch("/revolutionary.json")
                .then((res) => res.json())
                .then((data) => setRevolutionary(data));
        }, []);
        console.log(revolutionary);
        
    return (
        <div>
            <div className="grid grid-cols-1 sm:grid-cols-1 md:grid-cols-4 lg:grid-cols-4 gap-6">
                {revolutionary.map((martyr, index) => (
                    <motion.div
                        key={martyr.id}
            ...
            <div className="text-center">
                <Link to="projects/july-uprising/revolutionary"><button className="mt-6 bg-red-600 text-white px-6 py-3 rounded-lg hover:bg-red-700 transition">
                    See All Revolutionary
                </button></Link>
            </div>
        </div>
    );
